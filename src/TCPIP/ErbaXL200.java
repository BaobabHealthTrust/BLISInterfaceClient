 ...
 */
public class ErbaXL200 extends Thread {
    ...
    public void Stop() {
        try {
            ...
            welcomeSocket.close();
//            connSock.close();
            log.AddToDisplay.Display("Erba XL 200 handler stopped", DisplayMessageType.TITLE);
    ...
    @Override
    public void run() {
        log.AddToDisplay.Display("Erba XL 200 handler started...", DisplayMessageType.TITLE);
        ...
        try {
            ...
            log.AddToDisplay.Display("Erba XL 200 is now Connected...", DisplayMessageType.INFORMATION);
            first = false;
            ClientThread client = new ClientThread(connSock, "Erba XL 200");
            client.start();
            String message;
            ...
            if (first) {
                ...
                log.AddToDisplay.Display("Erba XL 200 client is now disconnected!", DisplayMessageType.WARNING);
                log.logger.Logger(e.getMessage());
            }
        ...
        try {
            ...
            if (SampleList.size() > 0) {
                for (int i = 0; i < SampleList.size(); i++) {
                    ...
                    log.AddToDisplay.Display("Sending test with Code: " + SampleList.get(i).aux_id + " to Erba XL 200", DisplayMessageType.INFORMATION);
    ...
    public static void handleMessage(String message) {
        try {
            ...
            String[] msgParts = message.trim().split("\r");
            if (type == MSGTYPE.QUERY) {
                ...
                if (pidParts.length > 5) {
                    ...
                    for (int i = 3; i < msgParts.length; i++) {
                        ...
                        if (msgParts[i].toString().trim().startsWith("R|")) {

                            mID = getMeasureID(msgParts[i].split("\\|")[2].split("\\^")[3]);
    ...
    private static String getSpecimenFilter(int whichdata) {
        String data = "";
        xmlparser p = new xmlparser("configs/Erba/erbaxl200.xml");
        try {
            data = p.getMicros60Filter(whichdata);
        } catch (Exception ex) {
            Logger.getLogger(ErbaXL200.class.getName()).log(Level.SEVERE, null, ex);
        }
        return data;
    }
