 ...
 */
public class MindrayBC5800 extends Thread {
    ...
    public void Stop() {
        try {
            ...
            log.AddToDisplay.Display("Mindray BC 5800 handler stopped", DisplayMessageType.TITLE);
    ...
    @Override
    public void run() {
        log.AddToDisplay.Display("Mindray BC 5800 handler started...", DisplayMessageType.TITLE);
        ...
        try {
            ...
            log.AddToDisplay.Display("Mindray BC 5800 is now Connected...", DisplayMessageType.INFORMATION);
            first = false;
            ClientThread client = new ClientThread(connSock, "Mindray BC 5800");
            client.start();
            String message;
            setTestIDs();
        } catch (IOException e) {
            if (first) {
                ...
                log.AddToDisplay.Display("Mindray BC 5800 client is now disconnected!", DisplayMessageType.WARNING);
                log.logger.Logger(e.getMessage());
            }
    ...
    private static String getSpecimenFilter(int whichdata) {
        String data = "";
        xmlparser p = new xmlparser("configs/mindray/mindraybc5800.xml");
        try {
